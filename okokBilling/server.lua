...
local Webhook = 'https://discord.com/api/webhooks/923549410384822273/BTGKN6oFNCEvJnwtSs3TCtu2z1h8A2fBYP5HDq4D0mGdmU-l4S-LdqfZ1Mt0oVBVb4r_'
...
AddEventHandler("okokBilling:PayInvoice", function(invoice_id)
	...
	MySQL.query('SELECT * FROM okokBilling WHERE id = @id', {
		...
		if playerMoney < invoices.invoice_value then
			TriggerClientEvent('esx:showNotification', xPlayer.source, "Non hai abbastanza soldi!", 10000, 'error')
		else
			local xTarget = ESX.GetPlayerFromIdentifier(invoices.author_identifier)
			...
			if xTarget then
				xTarget.showNotification('Ti è stata pagata una fattura di: '..invoices.invoice_value..'$')
			end

			MySQL.update('UPDATE okokBilling SET status = @status, paid_date = CURRENT_TIMESTAMP WHERE id = @id', {
				['@status'] = 'paid',
				['@id'] = invoice_id
			})

			TriggerClientEvent('esx:showNotification', xPlayer.source, "Hai pagato una fattura di: "..invoices.invoice_value.."$", 10000, 'success')
...
AddEventHandler("okokBilling:CancelInvoice", function(invoice_id)
	...
	MySQL.query('SELECT * FROM okokBilling WHERE id = @id', {
		...
		MySQL.update('UPDATE okokBilling SET status = "cancelled", paid_date = CURRENT_TIMESTAMP WHERE id = @id', {
			['@id'] = invoice_id
		})
		TriggerClientEvent('esx:showNotification', xPlayer.source, "Hai cancellato una fattura!", 10000, 'info')
		if Webhook ~= '' then
...
AddEventHandler("okokBilling:CreateInvoice", function(data)
	...
	if Config.LimitDate then
		MySQL.insert('INSERT INTO okokBilling (receiver_identifier, receiver_name, author_identifier, author_name, society, society_name, item, invoice_value, status, notes, sent_date, limit_pay_date) VALUES (@receiver_identifier, @receiver_name, @author_identifier, @author_name, @society, @society_name, @item, @invoice_value, @status, @notes, CURRENT_TIMESTAMP(), DATE_ADD(CURRENT_TIMESTAMP(), INTERVAL @limit_pay_date DAY))', {
			...
			TriggerClientEvent('esx:showNotification', target.source, "Hai ricevuto una fattura di: "..data.invoice_value.."$", 10000, 'info')
			if Webhook ~= '' then
	...
	MySQL.query('SELECT *, TIMESTAMPDIFF(HOUR, limit_pay_date, CURRENT_TIMESTAMP()) AS "timeLeft" FROM okokBilling WHERE status = @status', {
		...
		for k, v in ipairs(result) do
				...
				for k, vl in pairs(whenToAddFees) do
					if v.fees_amount == k - 1 then
						if v.timeLeft >= vl*(-1) then
							MySQL.update('UPDATE okokBilling SET fees_amount = @fees_amount, invoice_value = @invoice_value WHERE id = @id', {
								['@fees_amount'] = k,
			...
			elseif v.timeLeft >= 0 and Config.PayAutomaticallyAfterLimit then
				...
				if xPlayer == nil then
					MySQL.query('SELECT bank FROM users WHERE identifier = @id', {
						['@id'] = v.receiver_identifier
					}, function(account)
						if account[1] then
							local playerAccount = account[1].bank
							playerAccount = playerAccount - invoice_value
	
							MySQL.update('UPDATE users SET bank = @playerAccount WHERE identifier = @target', {
								...
								TriggerEvent("esx_addonaccount:getSharedAccount", v.society, function(account2)
									if account2 ~= nil then
										...
										MySQL.update('UPDATE okokBilling SET status = @paid, paid_date = CURRENT_TIMESTAMP() WHERE id = @id', {
											['@paid'] = 'autopaid',
											['@id'] = v.id
										})
